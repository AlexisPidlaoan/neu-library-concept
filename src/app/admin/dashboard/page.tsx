...
export default function LandingPage() {
  ...
  const handleIdChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 7) {
      formatted = formatted.slice(0, 2) + '-' + value.slice(2, 7) + '-' + value.slice(7)
    }
      ...
      <div className="w-full max-w-md relative z-10">
        <Card className="border-none shadow-2xl bg-white/95 overflow-hidden">
          <div className="bg-[#003399] p-8 text-center text-white relative">
            ...
            <h1 className="text-3xl font-bold mb-2 text-white">Welcome to NEU Library</h1>
          ...
          <CardContent className="p-8">
            {pendingStudentId ? (
              <div className="space-y-6 text-center animate-in fade-in zoom-in-95 duration-300">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400">
                    Please sign in with your institutional email to link this ID to your profile.
                  </p>
                </div>
              ...
              <form onSubmit={handleIdSubmit} className="space-y-6">
                <div className="space-y-2">
                  <Label htmlFor="student-id" className="text-xs font-bold text-slate-400 uppercase tracking-wider">STUDENT ID / EMPLOYEE ID</Label>
                  <Input 
                    id="student-id"
                ...
                <div className="pt-4 flex justify-center border-t border-slate-100 mt-4">
                  <Link 
                    href="/admin/login" 
                    className="text-slate-500 hover:text-primary text-xs flex items-center gap-1 transition-colors font-semibold"
                  >
